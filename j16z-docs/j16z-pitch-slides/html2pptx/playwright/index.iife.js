var _ExtractSlideData = ((exports) => {
  ...
  function balanceAllText(selector = '') {
    ...
    document.querySelectorAll(mergedSelector).forEach((element) => {
      ...
      const ratio = ratioAttr ? Number.parseFloat(ratioAttr) : 1;
      balanceText(element, !Number.isNaN(ratio) && ratio >= 0 && ratio <= 1 ? ratio : 1);
    });
  }
  ...
  function pxToPoints(pxStr) {
    return Number.parseFloat(String(pxStr)) * PT_PER_PX;
  }
  /**
  ...
  function getRotation(transform, writingMode) {
    ...
    if (transform && transform !== 'none') {
      ...
      if (rotateMatch) angle += Number.parseFloat(rotateMatch[1]);
      else {
  ...
  function parseBoxShadow(boxShadow) {
    ...
    if (colorMatch) {
      ...
      if (opacityMatch) opacity = Number.parseFloat(opacityMatch[0].replace(')', ''));
    }
    return {
      type: 'outer',
    ...
    element.childNodes.forEach((node) => {
      ...
      if (isText) {
        ...
        if (
          ...
          if (
            (computed.fontWeight === 'bold' || Number.parseInt(computed.fontWeight, 10) >= 600) &&
            !shouldSkipBold(computed.fontFamily)
          )
            options.bold = true;
          ...
          if (computed.marginLeft && Number.parseFloat(computed.marginLeft) > 0)
            errors.push(
          ...
          if (computed.marginRight && Number.parseFloat(computed.marginRight) > 0)
            errors.push(
          ...
          if (computed.marginTop && Number.parseFloat(computed.marginTop) > 0)
            errors.push(
          ...
          if (computed.marginBottom && Number.parseFloat(computed.marginBottom) > 0)
            errors.push(
  ...
  function getOverflowErrors(container, element, descriptor) {
    ...
    EDGES.forEach((edge) => {
      const amount =
        edge === 'bottom' || edge === 'right' ? element[edge] - container[edge] : container[edge] - element[edge];
      if (amount > 0) {
  ...
  function getElementOverlapError(rect1, rect2, descriptor1, descriptor2) {
    ...
    if (fullyOverlappedX && !fullyOverlappedY)
      return `The ${elementEdgeY} of ${descriptor1} overlaps with the ${otherEdgeY} of ${descriptor2} by ${yOverlap.toFixed()}px`;
    ...
    if (fullyOverlappedY && !fullyOverlappedX)
      return `The ${elementEdgeX} of ${descriptor1} overlaps with the ${otherEdgeX} of ${descriptor2} by ${xOverlap.toFixed()}px`;
    if (fullyOverlappedX && fullyOverlappedY) {
  ...
  const SLIDE_PADDING_PX = 0.25 * PX_PER_IN;
  /**
    ...
    function validate() {
      for (let i = 0; i < elements.length; i++) {
        ...
        for (let j = i + 1; j < elements.length; j++) {
          ...
          if (otherContentRect) {
            const error = getElementOverlapError(
              contentRect,
              otherContentRect,
              elementDescriptor,
  ...
  async function extractSlideDataInBrowser() {
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (SUPPORTED_TEXT_TAGS.includes(el.tagName)) {
        ...
        const hasBorder =
          (computed$1.borderWidth && Number.parseFloat(computed$1.borderWidth) > 0) ||
          (computed$1.borderTopWidth && Number.parseFloat(computed$1.borderTopWidth) > 0) ||
          (computed$1.borderRightWidth && Number.parseFloat(computed$1.borderRightWidth) > 0) ||
          (computed$1.borderBottomWidth && Number.parseFloat(computed$1.borderBottomWidth) > 0) ||
          (computed$1.borderLeftWidth && Number.parseFloat(computed$1.borderLeftWidth) > 0);
        const hasShadow = computed$1.boxShadow && computed$1.boxShadow !== 'none';
        if (hasBg || hasBorder || hasShadow) {
          errors.push(
            `Text element <${el.tagName.toLowerCase()}> has ${hasBg ? 'background' : hasBorder ? 'border' : 'shadow'}. Backgrounds, borders, and shadows are only supported on <div> elements, not text elements.`,
          );
          return;
        }
      }
      if (el.classList?.contains('placeholder')) {
        ...
        if (isSmallWidth || isSmallHeight)
          errors.push(
            `Placeholder "${el.id || 'unnamed'}" is too small: \`width: ${rect$1.width}px; height: ${rect$1.height}px;\`. Check the layout CSS.`,
          );
        else
          placeholders.push({
      ...
      if (SUPPORTED_BLOCK_ELEMENTS.includes(el.tagName)) {
        ...
        for (const node of Array.from(el.childNodes))
          if (node.nodeType === Node.TEXT_NODE) {
            ...
            if (text$1)
              errors.push(
                `${el.tagName} element contains unwrapped text "${text$1.substring(0, 50)}${text$1.length > 50 ? '...' : ''}". All text must be wrapped in <p>, <h1>-<h6>, <ul>, or <ol> tags to appear in PowerPoint.`,
              );
          }
        ...
        if (bgImage$1 && bgImage$1 !== 'none')
          if (bgImage$1.includes('linear-gradient') || bgImage$1.includes('radial-gradient')) {
        ...
        const borders = [borderTop, borderRight, borderBottom, borderLeft].map((b) => Number.parseFloat(b) || 0);
        ...
        if (hasBg || hasBorder || backgroundImagePath || backgroundGradient) {
          ...
          if (rect$1.width > 0 && rect$1.height > 0) {
            ...
            if (hasBg || hasUniformBorder || backgroundImagePath)
              elements.push({
                type: 'shape',
                text: '',
                position: rectToXYWH(rect$1),
      ...
      if (el.tagName === 'UL' || el.tagName === 'OL') {
        ...
        elements.push({
          ...
          style: {
            ...
            lineSpacing:
              computed$1.lineHeight && computed$1.lineHeight !== 'normal' ? pxToPoints(computed$1.lineHeight) : null,
            paraSpaceBefore: 0,
      ...
      if (el.tagName !== 'LI' && /^[•\-*▪▸○●◆◇■□]\s/.test(text.trimStart())) {
        errors.push(
          `Text element <${el.tagName.toLowerCase()}> starts with bullet symbol "${text.substring(0, 20)}...". Use <ul> or <ol> lists instead of manual bullet symbols.`,
        );
        return;
      }
      const computed = window.getComputedStyle(el);
      ...
      if (hasFormatting) {
        ...
        const isBold = computed.fontWeight === 'bold' || Number.parseInt(computed.fontWeight, 10) >= 600;
        elements.push({
          type: elementType,
