...
import path, { dirname, join, join as join$1, parse } from 'node:path';
...
async function addBackground(slideData, targetSlide, tmpDir, slideWidth = 10, slideHeight = 5.625) {
  if (slideData.background.type === 'image' && slideData.background.path) {
    ...
    if (path$1.includes('linear-gradient') || path$1.includes('radial-gradient'))
      targetSlide.background = {
        path: await generateGradientImage(path$1, Math.round(slideWidth * 96), Math.round(slideHeight * 96), tmpDir),
      };
...
const playwrightDir = existsSync(join$1(__dirname, 'playwright/index.css'))
  ? join$1(__dirname, 'playwright')
  : join$1(__dirname, '../dist/playwright');
/**
