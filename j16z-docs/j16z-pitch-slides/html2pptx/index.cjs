...
function getStyleForRasterizationType(rasterizationType, rasterizeId) {
  ...
  switch (rasterizationType) {
    ...
    case 'gradient':
      return `${baseStyle}[data-rasterize="${rasterizeId}"] {
          visibility: visible;
          box-shadow: none;
        }`;
    default:
      return `${baseStyle}svg, canvas, [data-rasterize] {visibility: visible;}
...
function parseAngle(angleStr) {
  ...
  if (degMatch) return Number.parseFloat(degMatch[1]);
  return 180;
}
/**
...
function parseColorStops(stopsStr) {
  ...
  parts.forEach((part, idx) => {
    ...
    if (match) {
      ...
      const offset = match[2] ? Number.parseFloat(match[2]) / 100 : idx / Math.max(1, parts.length - 1);
...
function normalizeColor(color) {
  ...
  if (rgbMatch) {
    ...
    const alpha = rgbMatch[4] ? Number.parseFloat(rgbMatch[4]) : void 0;
    return {
      color: `#${r}${g}${b}`,
      opacity: alpha,
    };
...
function generateRadialGradientSVG(position, stops, width, height) {
  ...
  if (position && position !== 'center') {
    ...
    if (parts[0]) {
      if (parts[0].endsWith('%')) cx = Number.parseFloat(parts[0]);
      else if (parts[0] === 'left') cx = 0;
      else if (parts[0] === 'right') cx = 100;
    }
    if (parts[1]) {
      if (parts[1].endsWith('%')) cy = Number.parseFloat(parts[1]);
...
async function generateGradientImage(gradientStr, width, height, tmpDir) {
  ...
  const hash = crypto.default
    .createHash('md5')
    .update(`${gradientStr}-${width}-${height}`)
    .digest('hex')
    .substring(0, 8);
...
async function addBackground(slideData, targetSlide, tmpDir, slideWidth = 10, slideHeight = 5.625) {
  if (slideData.background.type === 'image' && slideData.background.path) {
    ...
    if (path$4.includes('linear-gradient') || path$4.includes('radial-gradient'))
      targetSlide.background = {
        path: await generateGradientImage(path$4, Math.round(slideWidth * 96), Math.round(slideHeight * 96), tmpDir),
      };
...
async function addElements(slideData, targetSlide, pres) {
  for (const el of slideData.elements)
    if (el.type === 'image') {
      ...
      if (el.style.lineSpacing !== null && el.style.lineSpacing !== void 0)
...
async function getChromiumPath() {
  ...
  const expectedRevision = Number.parseInt(chromiumDirName.replace('chromium-', ''), 10);
  ...
  for (const dir of availableDirs) {
    const revision = Number.parseInt(dir.replace('chromium-', ''), 10);
...
const __dirname$1 = (0, node_path.dirname)(
  (0, node_url.fileURLToPath)(require('node:url').pathToFileURL(__filename).href),
);
const playwrightDir = (0, node_fs.existsSync)((0, node_path.join)(__dirname$1, 'playwright/index.css'))
  ? (0, node_path.join)(__dirname$1, 'playwright')
  : (0, node_path.join)(__dirname$1, '../dist/playwright');
/**
