...
import { AlertTriangle, Key, Mail, Rss, Users, Zap } from 'lucide-react';
import { useState } from 'react';
import { AlertRulesTab } from '@/components/settings/alert-rules-tab';
import { APIKeysTab } from '@/components/settings/api-keys-tab';
import { DigestPreferencesTab } from '@/components/settings/digest-preferences-tab';
import { IntegrationsTab } from '@/components/settings/integrations-tab';
import { RSSFeedsTab } from '@/components/settings/rss-feeds-tab';
import { TeamTab } from '@/components/settings/team-tab';

type SettingsTab = 'alerts' | 'digests' | 'integrations' | 'rss' | 'team' | 'api';
...
export default function SettingsPage() {
  ...
  const tabs = [
    ...
    { id: 'digests' as SettingsTab, label: 'Digests', icon: Mail },
  ...
  return (
    <div className="flex h-full flex-col p-6">
      <div className="border-b border-border pb-4">
        ...
        <p className="mt-1 text-sm text-text-muted">Configure alerts, integrations, and team settings</p>
      </div>
      ...
      <div className="mt-6 flex gap-6">
        ...
        <div className="flex-1 rounded-lg border border-border bg-surface p-6 transition-opacity duration-200">
          <div className="animate-in fade-in duration-200">
            ...
            {activeTab === 'digests' && <DigestPreferencesTab />}
