...
import { Bot, Globe, Link as LinkIcon, Loader2, Send, Terminal, User } from 'lucide-react';
    ...
    <div className="flex h-[calc(100vh-8rem)] flex-col overflow-hidden border border-border bg-background font-mono">
      <div className="flex items-center justify-between border-b border-border bg-surface p-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xs font-bold uppercase tracking-wider text-text-main">J16Z Analyst Agent</h2>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <span className={`h-3 w-3 bg-current ${isDeepResearch ? 'text-primary-500' : 'text-text-muted'}`} />
          </button>
        </div>
      </div>
      ...
      <div className="scrollbar-thin scrollbar-thumb-border flex-1 space-y-6 overflow-y-auto p-4">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            ...
            >
              {msg.role === 'user' ? <User className="h-3 w-3" /> : <Terminal className="h-3 w-3" />}
            </div>
            <div className={`max-w-[85%] space-y-1 ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>
              <div
                className={`inline-block px-4 py-3 text-xs leading-relaxed ${
                  msg.role === 'user' ? 'border border-border bg-surface text-text-main' : 'text-text-muted'
                }`}
              >
                {msg.isThinking && (
                ...
                <pre className="max-w-full whitespace-pre-wrap font-mono text-xs">{msg.content}</pre>
              </div>
      ...
      <div className="border-t border-border bg-surface p-4">
        <div className="group relative">
          <input
            ...
            placeholder={isDeepResearch ? 'INITIATE DEEP RESEARCH (MOCK)...' : 'QUERY DATABASE (MOCK)...'}
        ...
        <div className="mt-2 flex justify-between px-1 text-[10px] text-text-dim">
          <span className="uppercase">Input Mode: {isDeepResearch ? 'DEEP_THINK' : 'STANDARD'}</span>
