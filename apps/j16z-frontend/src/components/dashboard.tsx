    ...
    >
      <div className="mb-4 flex items-start justify-between">
        <div className="space-y-1">
          ...
          <h3 className="font-narrative text-3xl font-bold text-text-main">{value}</h3>
        </div>
...
const WatchlistItem = ({
  ...
  <div className="group flex items-center justify-between border-b border-border p-3 transition-colors hover:bg-surfaceHighlight">
    ...
    <div className="text-right">
      <div className="text-[10px] text-text-muted mb-1 uppercase tracking-wider">New Filings</div>
  ...
  return (
    <div className="space-y-6 font-mono">
      <div className="flex items-center justify-between border-b border-border pb-4">
        <div>
          <h1 className="font-narrative text-xl font-bold uppercase tracking-tight text-text-main">System Status</h1>
      ...
      <div className="grid grid-cols-1 gap-px border border-border bg-border md:grid-cols-2 lg:grid-cols-4">
        <MetricCard label="Ingestion Rate" value="14.2 MB/s" trend="+12%" icon={Activity} />
        <MetricCard label="Items Processed" value="2,491" trend="+540" icon={FileText} alertLevel="warning" />
        <MetricCard label="Pending Review" value="5" trend="+2" icon={Scale} />
        <MetricCard label="System Health" value="98.9%" trend="NOMINAL" icon={AlertTriangle} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
            ...
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={CHART_DATA}>
                  ...
                  />
                  <Area type="step" dataKey="docs" stroke="#f59e0b" fill="#f59e0b" fillOpacity={0.1} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="flex flex-col border border-border bg-background">
            ...
            <div className="divide-y divide-border">
              <WatchlistItem ticker="MSFT" name="Microsoft Corp" change={0.45} filings={2} />
              <WatchlistItem ticker="ADBE" name="Adobe Inc" change={-1.2} filings={1} />
              <WatchlistItem ticker="KRO" name="Kroger Co" change={0.12} filings={0} />
              <WatchlistItem ticker="ATVI" name="Activision Blizzard" change={0.05} filings={4} />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col border border-border bg-background p-0">
          <div className="border-b border-border bg-surface p-4">
            <h3 className="text-xs font-bold uppercase tracking-widest text-text-muted">Data Feeds</h3>
          </div>
          <div className="divide-y divide-border">
            {DATA_SOURCES.slice(0, 5).map((source) => (
              ...
              >
                <div>
                  <div className="text-xs font-bold uppercase text-text-main">{source.name}</div>
                  <div className="text-[10px] text-text-muted">{`${source.type.toUpperCase()} // ${source.id}`}</div>
                </div>
                <div className="text-right">
                  <div
                  ...
                  <div className="text-[10px] text-text-muted">{source.itemsToday} items</div>
                </div>
              </div>
            ))}
          </div>
